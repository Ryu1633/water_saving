...
ISR(TIMER2_OVF_vect) {
    ...
    if(timer2 >= 200) {
        TCCR2 &= TCCR2 & 0b11111000;    //타이머2 정지
        TIMSK &= TIMSK & 0b10111111;    //타이머2 인터럽트 비활성화
    }
}
...
void TIMER0_init(void) {
	...
	TCCR0 |= 1<<COM01;                  	//타이머0의 OCR과 TCNT가 일치하면 LOW, OVF발생시 HIGH
	OCR0 = 130;                             //1초당 평균전압 4.5V ~ 4.8V가 되는 OCR값, 9V의 55.55% duty = 5V
    //TIMSK |= 1<<TOIE0;                      //타이머0 인터럽트 활성화
}

//타이머/카운터2 초기화 함수
void TIMER2_init(void) {
    DDRB |= 1<<OC2;                 //PB7 출력핀 설정 = 서보모터 PWM 출력핀으로 설정
    TCCR2 = 1<<WGM21 | 1<<WGM20;    //타이머2 고속 PWM 모드
    TCCR2 |= 1<<COM01;              //타이머2의 OCR과 TCNT가 일치하면 LOW, OVF발생시 HIGH
    OCR2 = (int)34;                 //서보모터 각도 초기값 = 냉수방향
}

//ADC초기화 함수
void ADC_init(void) {
    ADCSRA |= 1<<ADEN | 1<<ADIE | 7;    //ADC 활성화(1<<ADEN), ADC인터럽트 활성화(1<<ADIE), 128분주 = ADCSRA = 1000 1111
	ADCSRA |= 1<<ADFR;                  //ADC 연속모드 설정 =   ADCSRA = 1010 1111
}
    ...
    while(1) {
        switch(state) {
            case IDLE:                                      //대기모드
                //동작 인식하면 급수 시작
                if(adc_result[0] > 500) {        
                    TCCR0 |= 1<<CS02 | 1<<CS01 | 1<<CS00;	//타이머0 작동 시작, 1024분주, 주기 = 0.016384, 61Hz
                    TIMSK |= 1<<TOIE0;                      //타이머0 인터럽트 활성화
...
//##################################### ADC채널 변경 #####################################
		        ...
		        ADMUX = channel == SENSOR0 ? 0b00000000 : (channel == SENSOR1 ? 0b00000001 : 0b00000010);
...
//##################################### 대기모드 전환 #####################################
                ...
                if(cnt >= 300) {
                    ...
                    ADMUX = 0b00000000;         //PIR센서 채널로 변경을 위해 ADMUX 레지스터 변경
                    timer0_rq = 1;              //타이머0 인터럽트 요청
                    while(timer0_rq);           //타이머/카운터0 인터럽트 발생까지 대기 = 16ms대기 = ADC 채널변경을 위한 시간지연
                    DDRB &= DDRB & 0b10000000;  //타이머0 PWM 신호 출력 중지 = 워터펌프 정지
                    TCCR0 &= TCCR0 & 0b11111000;    //타이머2 정지
                    TIMSK &= TIMSK & 0b10111111;    //타이머2 인터럽트 비활성화
                }//end if
//########################################################################################
		...
		while(adc_rq);		// ADC 인터럽트 응답을 기다림
		

//##################################### 컴파일용 #####################################
		sprintf(lcd_string[0], "%-4d", adc_result[0]);  //PIR 센서값 배열에 저장
		LCD_str_write(0, 5, lcd_string[0]);             //PIR 센서값 LCD에 출력

        sprintf(lcd_string[3], "%-4d", cnt);         //컴파일 결과 확인용 배열에 저장
		LCD_str_write(0, 13, lcd_string[3]);             //컴파일 결과 확인용 LCD에 출력

        sprintf(lcd_string[1], "%-4d", adc_result[1]);  //왼쪽 적외선 센서값 배열에 저장
		LCD_str_write(1, 5, lcd_string[1]);              //왼쪽 적외선 센서값 LCD에 출력

        sprintf(lcd_string[2], "%-4d", adc_result[2]);  //오른쪽 적외선 센서값 배열에 저장
		LCD_str_write(1, 13, lcd_string[2]);             //오른쪽 적외선 센서값 LCD에 출력
//########################################################################################
    }   //end while
    return 0;
}